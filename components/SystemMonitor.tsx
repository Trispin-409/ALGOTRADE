    ...
    <div className="space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full overflow-x-hidden">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="glowing-panel p-6 sm:p-8 rounded-[30px] sm:rounded-[40px] shadow-2xl flex items-center gap-4 sm:gap-6 overflow-hidden bg-black/40">
          <div className="w-10 h-10 sm:w-12 h-12 rounded-2xl flex items-center justify-center border border-white/10 shrink-0 bg-black/40 accent-glow" style={{ color: 'var(--accent-color)' }}>
        ...
        <div className="glowing-panel p-6 sm:p-8 rounded-[30px] sm:rounded-[40px] shadow-2xl flex items-center gap-4 sm:gap-6 overflow-hidden sm:col-span-2 lg:col-span-1 bg-black/40">
          <div className="w-10 h-10 sm:w-12 h-12 rounded-2xl flex items-center justify-center border border-white/10 shrink-0 bg-black/40 accent-glow" style={{ color: 'var(--accent-color)' }}>
      ...
      <div className="bg-[#050505] border border-white/10 rounded-[30px] sm:rounded-[40px] overflow-hidden shadow-2xl flex flex-col h-[400px] sm:h-[600px] w-full glowing-frame">
        <div className="bg-black/80 px-6 sm:px-8 py-4 border-b border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
          <div className="flex items-center gap-3">
            <Terminal className="w-3.5 h-3.5 sm:w-4 h-4 shrink-0" style={{ color: 'var(--accent-color)' }} />
            <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest truncate" style={{ color: 'var(--accent-color)' }}>Telemetry Trace</span>
          </div>
        ...
        <div 
          ref={scrollRef}
          className="flex-1 overflow-y-auto p-4 sm:p-10 font-mono text-[10px] sm:text-[11px] leading-relaxed custom-scrollbar flex flex-col gap-1 bg-black/90"
        >
          {logs.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center gap-4 opacity-20">
              <Loader2 className="w-6 h-6 sm:w-8 h-8 animate-spin shrink-0" style={{ color: 'var(--accent-color)' }} />
              <p className="uppercase tracking-[0.2em] sm:tracking-[0.5em] font-black text-[8px] sm:text-[9px] text-center px-4" style={{ color: 'var(--accent-color)' }}>Handshake in progress...</p>
            </div>
          ) : (
            logs.map((log, i) => {
              ...
              return (
                <div key={i} className={`flex items-start gap-3 sm:gap-4 animate-in slide-in-from-left-2 duration-300 py-1 sm:py-0.5 border-l-2 pl-3 sm:pl-4 transition-all ${
                  ...
                  isSuccess ? 'border-emerald-500/50 bg-emerald-500/5' : 'border-white/5'
                }`}>
        ...
        <div className="bg-black/80 px-6 sm:px-8 py-3 sm:py-4 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between text-[7px] sm:text-[9px] font-black text-slate-600 uppercase tracking-widest gap-2 sm:gap-0 shrink-0">
          <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto justify-between sm:justify-start">
            <span className="flex items-center gap-1.5">
              <Activity className="w-2.5 h-2.5 sm:w-3 h-3 text-white/20 shrink-0" />
              <span className="truncate">Synced</span>
            </span>
            <span className="flex items-center gap-1.5">
              <Zap className="w-2.5 h-2.5 sm:w-3 h-3 text-white/20 shrink-0" />
              <span className="truncate">42ms Latency</span>
            </span>
          </div>
          <div className="flex items-center gap-1.5 text-white/10 truncate">
      ...
      <div className="glowing-panel p-6 sm:p-8 rounded-[30px] sm:rounded-[40px] shadow-2xl backdrop-blur-md space-y-6 w-full overflow-hidden bg-black/40">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 font-mono text-[8px] sm:text-[10px]">
          <div className="p-3 sm:p-4 bg-black/40 rounded-xl sm:rounded-2xl border border-white/5 overflow-hidden">
            ...
            <p className="font-bold tracking-widest uppercase truncate" style={{ color: 'var(--accent-color)' }}>SDK-CORE</p>
          </div>
          <div className="p-3 sm:p-4 bg-black/40 rounded-xl sm:rounded-2xl border border-white/5 overflow-hidden">
          ...
          <div className="p-3 sm:p-4 bg-black/40 rounded-xl sm:rounded-2xl border border-white/5 overflow-hidden">
            ...
            <p className="font-bold tracking-widest uppercase truncate" style={{ color: 'var(--accent-color)' }}>{safeToken ? safeToken.slice(0, 8) : '...'}...</p>
          </div>
        </div>
      </div>
    </div>
