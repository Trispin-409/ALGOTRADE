    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 w-full pb-20">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-1 space-y-6">
          ...
          <div className="glowing-panel rounded-xl p-5 shadow-lg backdrop-blur-md relative overflow-hidden bg-black/40">
            ...
            <div className="flex flex-col items-center justify-center p-6 bg-black/40 rounded-lg border border-white/10 mb-4 relative z-10">
          ...
          <div className="glowing-panel rounded-xl p-5 shadow-lg backdrop-blur-md bg-black/40">
          ...
          <div className="glowing-panel rounded-xl p-5 shadow-lg backdrop-blur-md bg-black/40">
             <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[8px] px-1.5 py-0.5 rounded bg-white/5 text-white border border-white/10 font-mono tracking-widest uppercase">FRED API</span>
             </div>
             <p className="text-[10px] font-mono text-slate-400 mb-4 tracking-widest uppercase border-b border-white/10 pb-2">{config.focus}</p>
             
             {loadingFred ? (
                 <div className="py-8 flex justify-center" style={{ color: 'var(--accent-color)' }}><Loader2 className="w-5 h-5 animate-spin" /></div>
             ) : (
                 <div className="space-y-3">
                     {FRED_SERIES.map(s => {
                          ...
                          return (
                              <div key={s.id} className="p-3 bg-black/40 rounded-lg border border-white/10 flex items-center justify-between group transition-colors hover:border-white/20">
                                  <div>
        ...
        <div className="xl:col-span-2">
           <div className="glowing-panel rounded-xl p-5 shadow-lg backdrop-blur-md h-full flex flex-col bg-black/40">
                ...
                <div className="flex items-center gap-2 overflow-x-auto custom-scrollbar pb-1 sm:pb-0">
                  {['ALL', 'USD', 'EUR', 'GBP', 'JPY', 'XAU'].map(c => (
                    <button 
                      ...
                      className={`px-3 py-1.5 text-[9px] font-mono font-bold uppercase tracking-widest rounded-md shrink-0 transition-all active:scale-95 border ${currencyFilter === c ? 'text-white border-white/20' : 'bg-black/60 text-slate-400 border-white/10 hover:text-white hover:bg-white/5'}`}
                      style={currencyFilter === c ? { backgroundColor: 'var(--accent-color)' } : {}}
                    >
                      {c}
                    </button>
                  ))}
              ...
              {loadingNews ? (
                <div className="flex flex-col items-center justify-center py-32 text-slate-500 flex-1">
                  <Loader2 className="w-8 h-8 animate-spin mb-4" style={{ color: 'var(--accent-color)' }} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 content-start">
                  {filteredNews.slice(0, 30).map((item, i) => (
                    <a 
                      ...
                      className="bg-black/40 border border-white/10 rounded-lg overflow-hidden transition-all group flex flex-col h-full hover:border-white/30"
                    >
                      {item.image && (
                        <div className="h-32 w-full overflow-hidden bg-black border-b border-white/10 shrink-0">
                          <img 
                            src={item.image} 
                            alt={item.headline}
                      ...
                      <div className="p-4 flex flex-col flex-1">
                        ...
                        <div className="flex items-center gap-1 text-[9px] font-black uppercase tracking-widest mt-auto opacity-50 group-hover:opacity-100 transition-opacity" style={{ color: 'var(--accent-color)' }}>
